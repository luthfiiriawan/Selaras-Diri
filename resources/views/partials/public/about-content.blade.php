...
<section class="mx-auto block w-[min(1160px,calc(100%-48px))] pt-16 reveal" id="tentang">
    <div class="mx-auto max-w-[920px]">
        ...
        <h1 class="mb-0 max-w-[720px] text-5xl leading-[1.08]">{{ $settings['about_title'] }}</h1>
        <p class="mt-6 text-[1.08rem] leading-[1.72] text-sd-muted">{{ $settings['about_body'] }}</p>
    </div>
</section>

<section class="mx-auto grid w-[min(1160px,calc(100%-48px))] gap-4 py-24 reveal-group lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)]">
    <article class="rounded-lg border border-sd-ink/10 bg-sd-primary p-8 text-sd-surface reveal">
        ...
        <h2 class="mt-2 text-4xl leading-tight">{{ $settings['vision_body'] }}</h2>
    </article>
